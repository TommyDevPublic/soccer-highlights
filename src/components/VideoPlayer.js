...
class VideoPlayer extends React.Component {
  ...
  renderContent = () => {
    ...
    const video = _.find(this.props.videos, v => {
      return v.competition.id === parseInt(this.props.match.params.id);
    });
                  ...
                  {video.videos.map((vid, i) => {
                    return (
                      <div key={`related-video-${i}`}>
                        <li>
                          {i + 1 ===
                          parseInt(this.props.match.params.relatedId) ? (
                            <span>
                            ...
                            <Link
                              to={`/soccer/videos/${moment(video.date).format(
                                "YYYY-MM-DD"
                              )}/${video.competition.id}/${slugify(vid.title, {
                                remove: /[*+~.()'"!:@]/g
                              })}/${i + 1}`}
                            >
                              {vid.title}
  ...
  render() {
    ...
    return (
      <div>
        <ModalWrapper content={this.renderContent()} />
      </div>
    );
  }
}
